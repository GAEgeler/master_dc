...
#try to plot that
p <- ggplot(df_f, aes(x = as.factor(fehlender_zahn_fdi_quadrant_zahnzahl), fill = gschlecht_m_w, y = pct_number_missing)) +
  ...
  scale_fill_manual(values = wes_palette("Royal2")) +
  labs(x = "Fehlender Zahn pro Quadrant??", y = "Anteil fehlende Zähne nach Geschlecht") +
  mytheme
...
#save
ggsave(plot = p, 
       filename = "output/plots/202207_missing_teeth_sex.pdf",
       device = cairo_pdf,
       width = 11,
       height = 7)
